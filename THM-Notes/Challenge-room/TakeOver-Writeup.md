...
- **Main Technique:** Subdomain Enumeration, SSL Certificate Inspection, Subdomain Takeover
...
Saya juga mencoba wordlist yang lebih besar (`subdomains-top1million-20000.txt` dan `subdomains-top1million-110000.txt`) dengan hasil yang sama.

### Manual Enumeration — Membaca Hint dari Deskripsi Room

Karena brute force tidak berhasil, saya kembali membaca deskripsi room dengan teliti. Di sana disebutkan bahwa perusahaan sedang *"rebuilding their **support**"* — ini adalah petunjuk bahwa kemungkinan ada subdomain bernama `support`.
...
Di sana ditemukan subdomain tersembunyi:
...
Celah utamanya adalah **Subdomain Takeover**. DNS record dari subdomain `secretxxxxxxx.thm` masih aktif dan mengarah ke layanan **AWS S3** yang sudah tidak dikonfigurasi (bucket tidak ada atau sudah dihapus). Kondisi ini memungkinkan penyerang untuk:
