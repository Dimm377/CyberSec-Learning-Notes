...
Active Directory (AD) itu tulang punggung jaringan korporat yang bikin manajemen terpusat buat seluruh sumber daya di jaringan jadi mungkin. Ngerti struktur AD itu penting banget buat praktisi keamanan biar paham gimana identitas dan akses dikelola dalam skala besar.
...
- **Organizational Units (OUs):** Kontainer logis yang dipake buat ngelompokin user, grup, dan komputer biar gampang nerapin kebijakan.
...
Ngelola objek di AD melibatkan alat administratif kayak _Active Directory Users and Computers_.

- **Object Protection:** Buat hapus OU yang terproteksi, fitur "Advanced Features" harus diaktifin biar opsi "Protect object from accidental deletion" bisa dinonaktifin.
...
- **Delegation:** Fitur yang bikin admin bisa ngasih hak spesifik ke user biasa (kayak reset password) tanpa harus ngasih hak administratif penuh.
...
AD pake dua protokol utama buat proses login dan akses sumber daya.

- **Kerberos:** Protokol default yang pake sistem tiket (Ticket Granting Ticket/TGT) buat meminimalisir pengiriman credensial di jaringan.
- **NTLM:** Protokol challenge-response yang lebih tua tapi masih sering dipake buat kompatibilitas ke belakang.
- **Security Fact:** Baik Kerberos maupun NTLM dirancang biar password dalam bentuk teks asli (plaintext) gak pernah dikirim lewat jaringan.

---

## Domain Trusts

Trust relationship bikin pengguna dari satu domain bisa ngakses sumber daya di domain lain.
